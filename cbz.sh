...
compression_flag="9" #9 for the highest compression level
#charset_flag="unicode=UTF8"
#open_flag="false" #open the file after creating
disable_xterm_flag="x" #disables xterm/OSC features, such as when being used in an app
exclude=("*.DS_Store" "*[Tt]humbs.db")
count=0 #number of archives zipped
zip_flags="" #combined zip flags so concatenation only occurs once.
...
zip_flags="-${remove_flag}${recursive_flag}${test_flag}${symlink_flag}${compression_flag}"
...
do
	...
	then
        ...
        count+=1
				...
				echo -ne "${tabtitle}Archiving \"$target\"…\a" #set tab title
        ...
        echo -ne ${style_flag}"${zip_flags}"${reset_style_flag}" " #flags
        echo -ne \"${style_file}"$archive.cbz"${reset_style_file}"\" " #comic book zip archive
        echo -ne \"${style_dir}"$target"${reset_style_dir}"\" " #source
				#echo -ne ${style_flag}"--${charset_flag}"${reset_style_flag}" " #character set flag
        ...
        echo $reset
				...
				zip ${zip_flags} "$archive.cbz" "$target" -x $exclude
